...
As an administrator, you can customize the following items in the *Admin Settings*:
...
* *Users*: Create and delete users, add or remove groups and roles from users, and view users' permissions.
...
.Prerequisites
* Installed {EAP_LONG} {EAP_VERSION_LONG}. For more information, see https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/7.2/html-single/installation_guide/[_{EAP_LONG} {EAP_VERSION} Installation Guide_].
* Installed {PRODUCT}. For more information, see {URL_INSTALLING_ON_EAP}[_{INSTALLING_ON_EAP}_].
* A running instance of {PRODUCT}. You must be able log in to {CENTRAL} with the `admin` role.
...
include::{enterprise-dir}/admin-and-config/environment-mode-con.adoc[leveloffset=+1]

include::{enterprise-dir}/admin-and-config/kie-server-configuring-environment-mode-proc.adoc[leveloffset=+2]

include::{enterprise-dir}/admin-and-config/business-central-configuring-environment-mode-proc.adoc[leveloffset=+2]
