...
In {CENTRAL}, each task in a process diagram has a `WorkItemDefinition` Java class with an associated `WorkItemHandler` Java class. The work item handler contains Java code registered with {CENTRAL} and implements `org.kie.api.runtime.process.WorkItemHandler`.
