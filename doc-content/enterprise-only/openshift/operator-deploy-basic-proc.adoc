...
. Optional: If you want to use a secret containing the credentials for the Admin user, under *Admin user configuration*, enter the secret configuration to use for the Admin user in the *Secret* field.
+
[NOTE]
====
If the *Secret* field is left blank, the `kie-admin-credentials` secret is used.
