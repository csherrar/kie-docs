...
.. Under *OpenShift admin credentials secret*, in the *Secret* field, enter the name of the secret you want to use. If the *Secret* field is left blank, the default secret from `kie-admin-credentials` is used.
.. Under *OpenShift admin credentials secret*, in the *Username* field, enter the username for the admin user. If the *Username* field is left blank, the default username from `kie-admin-credentials` is used.
.. Under *OpenShift admin credentials secret*, in the *Password* field, enter the password for the admin user. If the *Password* field is left blank, the default password from `kie-admin-credentials` is used.
+
[NOTE]
====
