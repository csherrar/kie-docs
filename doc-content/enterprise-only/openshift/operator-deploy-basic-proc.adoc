...
+
--
To use a specific image from the custom registry, set the image context, image name, and image tag parameters in the *Console* and *KIE Server* tabs:
...
You can specify an external image registry and image in a KieApp CR yaml file, for example:
