...
. Optional: To configure {KIE_SERVER} for decision management only capabilities so that jBPM and case management features are disabled, complete either one of the following steps:
+
* Select *Execute Kie Server only with Decisions capabilities*.
+
* Set `KIE_SERVER_DECISIONS_ONLY` to `true` in the `image.yaml` file. For more information, see https://github.com/jboss-container-images/rhpam-7-openshift-image/blob/main/kieserver/image.yaml#L263)[`image.yaml`] on GitHub.
+
[NOTE]
====
`KIE_SERVER_DECISIONS_ONLY` is set to `true` by default for Red Hat Decision Manager templates. For more information, see https://github.com/jboss-container-images/rhpam-7-openshift-image/blob/main/templates/decision/rhdm713-authoring-ha.yaml#L61).[`rhdm713-authoring-ha.yaml`] on GitHub.
====
