...
. Optional: To configure Mutual TLS (mTLS) authentication, that uses client certificates rather than credentials or tokens, configure `client-cert` for mTLS, for example:
+
----
kieservers:
