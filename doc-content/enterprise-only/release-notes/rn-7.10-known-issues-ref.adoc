...
Issue: When you use the *Data I/O* variables such as `nodeInstance`, `event`, and `workItemDefinition`, a reusable sub-process does not return any implicit data. All the returned values are null.
