...
--
+
. Optionally, once {KIE_SERVER} has started, you can send a GET request to `http://_SERVER:PORT_/kie-server/services/rest/server/` to verify that {KIE_SERVER} started successfully. For more information about running {PRODUCT} on {KIE_SERVER}, see {URL_INSTALLING_AND_CONFIGURING}#eap-ba-dm-run-proc_install-on-eap[_Running {PRODUCT}_].
+
After authentication, you receive an XML response similar to the following example:
+
[source,xml]
----
