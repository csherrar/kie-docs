...
[NOTE]
====
If you are using PostgreSQL or Oracle in conjunction with Spring Boot, you must instead import the respective Spring Boot DDL script, for example `/ddl-scripts/oracle/oracle-springboot-jbpm-schema.sql` or `/ddl-scripts/postgresql/postgresql-springboot-jbpm-schema.sql`.
====
