...
= Configuring a {EAP} 7.2 domain-mode cluster for {CENTRAL}
Configure the {CENTRAL} cluster in domain mode with Red Hat JBoss Data Grid and the JMS broker resource adapter integrated with {EAP}. By using domain mode with a cluster, you can centrally manage and publish configurations for your servers.
...
* `<INFINISPAN_NODE_IP>` is the IP addresses where Red Hat JBoss Data Grid is installed.
