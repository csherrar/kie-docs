...
. To start Smart Router with HTTPS enabled, use the TLS keystore properties, for example:
+
----
...
----
+
`org.kie.server.router.tls.port` is an optional setting to configure the HTTPS port. The default HTTPS port value is `9443`.
