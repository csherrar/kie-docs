...
If you send a request, the request is sent to `kie-server1 (129.0.0.2)`, `kie-server2 (129.0.1.3)`, and `kie-server3 (129.0.2.1)`.

If you send a second request, that request is sent to the next host of each {KIE_SERVER}. For example, `kie-server1 (129.0.0.3)`, `kie-server2 (129.0.1.1)`, and `kie-server3 (129.0.2.2)`.
...
----
java -cp LOCATION/router-ext-0.0.1-SNAPSHOT.jar:rhpam-7.7-smart-router.jar org.kie.server.router.KieServerRouter
----
