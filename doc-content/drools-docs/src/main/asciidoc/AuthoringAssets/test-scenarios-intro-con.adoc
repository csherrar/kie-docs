...
You can run the defined test scenarios in a number of ways, for example, you can run available test scenarios at the project level or inside a specific test scenario asset. Test scenarios are independent and cannot affect or modify other test scenarios. You can run test scenarios at any time during project development in {CENTRAL}. You do not have to compile or deploy your decision service to run test scenarios. For more information about running test scenarios, see {URL_TESTING_DECISION_SERVICE}/test-designer-run-test-proc[_{TESTING_DECISION_SERVICE}_].
