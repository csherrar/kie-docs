...
----
+
The YAML file creates a broker named `default` in the current namespace. For more information about configuring broker options using YAML, see https://knative.dev/docs/eventing/broker/example-mtbroker/[Broker configuration example - Knative].
...
|True
|
|http://broker-ingress.knative-eventing.svc.cluster.local/event-example/default |1m
|===
+
...
. To create the YAML files `knative.yml` and `kogito.yml`, enter the following:
+
----
mvn clean install -Pknative -Dnamespace=<your namespace>
----

. To deploy the objects from the YAML files `knative.yml` and `kogito.yml`, enter the following command.
+
----
