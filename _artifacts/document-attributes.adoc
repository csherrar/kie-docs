...
:REBUILT: Wednesday, May 5, 2021


:ENTERPRISE_VERSION: 7.11
:ENTERPRISE_VERSION_LONG: 7.11.0
...
ifdef::DM,PAM[]
:MAVEN_ARTIFACT_VERSION: {COMMUNITY_VERSION_FINAL}-redhat-00004
:BOM_VERSION: {ENTERPRISE_VERSION_LONG}.redhat-00004
endif::[]
...
:CUSTOM_TASKS_AND_HANDLERS: Custom tasks and work item handlers in {CENTRAL}
...
:RUNNING_VRP_FOR_PLANNER: Deploying and using the vehicle route planning starter application for {PLANNER}
...
:DEPLOYING_OPENSHIFT_TEMPLATES: Deploying a {PRODUCT} environment on {OPENSHIFT} using templates
